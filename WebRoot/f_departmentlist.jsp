	...
	<div class="mt-20">
		<table class="table table-border table-bordered table-hover table-bg table-sort">
			...
			<tr class="text-c">
				    ...
				    <th>最后修改时间</th>
                    <th>
						<button type="button" class="btn btn-success <pk:ruleControl module="f_department_add"/>  " name="" onclick="add()"><i class="Hui-iconfont"></i> 添加</button>
					</th>
                </tr>
			</thead>
			...
			<tbody>
              <c:forEach items="${data.list}" varStatus="i" var="row">
              	<tr class="text-c">
                    ...
                    <button class="btn btn-small btn-primary  <pk:ruleControl module="f_department_update"/>" type="button" onclick="update2(${row.id })">修改二级部门</button>
                    </td>
              	</tr>
              </c:forEach>
			</tbody>
...
<div class="page-container" id="edit2" style="display: none;">
	<form action="#" method="post" class="form form-horizontal" id="form-article-add">
			<input type="hidden" id="id">
	        <div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>二级部门名称：</label>
			<div class="formControls col-xs-8 col-sm-9">
...
function update2(id) {
	layer.open({
	      ...
	      maxmin: true, //开启最大化最小化按钮
	      area: ['100%', '100%'],
	      content: 'f_department.htm?operate=sonlist&parentid='+id,
	      zIndex:999
	});
}
//启用
