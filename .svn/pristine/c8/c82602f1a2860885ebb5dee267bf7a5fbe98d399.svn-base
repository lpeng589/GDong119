...
<body>
    ...
    <c:if test="${bean.flag }">
    <c:if test="${bean.logstatus==2}">
	    <div class="modify-type">
	        <span>处理类型</span> 
	        <select id="deal_status" onchange="isreturn()">
	          	<option value="10">正常提交</option>
	          	<option value="-1">移交</option>
	          	<option value="6">申请结案</option>
	          	<option value="11">非负责区域</option>
	          	<option value="8">无效案件</option>
	          	<option value="7">需整改</option>
	        </select>
	    </div>
	    <div class="hide" id=returndiv>
	    <div class="modify-type">
	        <span>移交人员</span>
	        <select id="next_id">
	        <option value="">请选择移交对象</option>
	        	<c:forEach items="${workuserlist}" var="row">
	        		<c:if test="${workuserid!=row.id }">
...
function orderAppend(data){
 	for(i=0;i<data.length;i++){
		...
		list = "<li><div class=\"modify-time\">"+item.createtime2+"<div class=\"circle-one\"><div class=\"circle-two\"></div></div></div>"
                ...
                +"<div class=\"modify-time-one\">"+item.createtime.substring(item.createtime.length-8, item.createtime.length)+"</div>"
                +"</div>"
                +deal_opinion
                +deal_fail
            	+"</div></li>";
...
function add(){
	...
	var callbackfun=function(data){
		...
		if($("#deal_status").val()==-1){
			...
			//询问框
			  layer.open({
			    content: '是否将案件移交给'+$("#next_id").find("option:selected").text()+"?"
			    ,btn: ['确定', '取消']
			    ,yes: function(index){
			    	commitData(url,obj,callbackfun);
			    }
